...
public class WallGameDiv2 {
    ...
    int play(String[] costs){
        int cost = 0;

        int width = costs[0].length();
        int height = costs.length;
        int[][] gridCost = new int[height][width];
        for(int i=0;i<height;i++){
            String costRow = costs[i];
            for(int j=0;j<width;j++){
                char d = costRow.charAt(j);
                gridCost[i][j] = d=='x'?MAX:d-'0';
            }
        }

        int[][] gridTotal = new int[height][width];
        for (int[] row : gridTotal){
            Arrays.fill(row,MIN);
        }

        gridTotal[0][0] = 0;
        for (int i=1;i<width;i++){
            if(gridCost[0][i]==MAX) break;
            gridTotal[0][i] = gridTotal[0][i-1]+gridCost[0][i];
        }

        int[][] gridDistanceInRow = new int[width][width];
        for (int i=1;i<height-1;i++){
            for(int[] row :gridDistanceInRow){
                Arrays.fill(row,MIN);
            }
            for (int j=0;j<width;j++){
                if(gridCost[i][j]==MAX) continue;
                gridDistanceInRow[j][j] = gridCost[i][j];
            }
            for (int j=0;j<width;j++){
                if(gridCost[i][j]==MAX) continue;
                for (int k=j+1;k<width;k++){
                    if(gridCost[i][k]==MAX) continue;
                    gridDistanceInRow[j][k] = gridDistanceInRow[k][j] = gridDistanceInRow[j][k-1]+gridCost[i][k];
                }
            }
            for(int j=0;j<width;j++){
                if(gridCost[i][j]==MAX) continue;
                for (int k=0;k<width;k++){
                    gridTotal[i][j] = Math.max(gridTotal[i][j],gridTotal[i-1][k]+gridDistanceInRow[k][j]);
                }
            }
            for (int j=0;j<width;j++){
                if(gridTotal[i][j]<0) gridTotal[i][j] = MIN;
            }
        }

        for (int i=0;i<width;i++){
            if(gridCost[height-1][i]!=MAX){
                cost = Math.max(cost,gridTotal[height-2][i]+gridCost[height-1][i]);
            }
        }
    ...
    public static void main(String[] args){
        System.out.println(new WallGameDiv2().play(new String[]
                        {
//                                "0"
//                                ,"5"
//                                "042"
